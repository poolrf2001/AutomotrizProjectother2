...
async function markOutboxSent(outboxId) {
  try {
    ...
    console.error("[outbox] markOutboxSent failed for id", outboxId, error.message);
    throw error;
  }
}
...
async function markOutboxFailed(outboxId, { retryCount, maxRetries, reason }) {
  ...
  await db.query(
    ...
    console.error("[outbox] markOutboxFailed failed for id", outboxId, error.message);
    throw error;
  }
}
...
async function fetchOutboxById(outboxId) {
  try {
    ...
    console.error("[outbox] fetchOutboxById failed for id", outboxId, error.message);
    throw error;
  }
}
...
async function acquireOutboxItem(outboxId) {
  ...
  try {
    ...
    await conn.query(
      `UPDATE conversations_outbox SET status = 'processing', updated_at = NOW() WHERE id = ?`,
      [outboxId]
    );
    await conn.commit();
    return rows[0];
